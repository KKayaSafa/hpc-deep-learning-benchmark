...
from torchvision.models import vgg19 # Changed to VGG19
...
class LitVGG19(L.LightningModule): # Renamed class for VGG19
    def __init__(self, learning_rate=1e-2, num_classes=10, optimizer_name="sgd",
        ...
        # Load VGG19
        weights_path = './model-weights/vgg19_weights.pth' # Updated weights path
        model = vgg19(weights=None) # Load base VGG19 model
        ...
        if os.path.exists(weights_path):
            try:
                ...
                print(f"Loaded VGG19 weights from {weights_path}")
            except Exception as e:
                print(f"Warning: Could not load weights from {weights_path}: {e}")
                print("Using randomly initialized model.")
        else:
            print(f"Warning: VGG19 weights not found at {weights_path}. Using randomly initialized model.")
        
        # Modify the classifier for CIFAR10 (10 classes)
        # VGG19 has a classifier with 3 linear layers, we need to modify the last one
        num_features = model.classifier[6].in_features  # Last layer in VGG classifier
...
def main():
    ...
    transform = transforms.Compose([
        transforms.Resize(224),  # VGG19 expects 224x224 images (standard ImageNet size)
        transforms.ToTensor(),
    ...
    model = LitVGG19(learning_rate=args.learning_rate,
    ...
    logger = TensorBoardLogger(save_dir="./lightning_logs/", name="experiments_vgg19", default_hp_metric=False)
    device_stats = DeviceStatsMonitor()
    gpu_logger = MultiGPUUtilizationLogger(log_frequency=100)

    checkpoint_dir = os.path.join("./out", f"checkpoints_vgg19_{args.strategy}_{args.optimizer}_bs{args.batch_size}_lr{args.learning_rate}_sch{args.lr_scheduler_name}")
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="vgg19-{epoch:02d}-{val_acc:.2f}"
    )
    ...
    if checkpoint_callback.best_model_path and os.path.exists(checkpoint_callback.best_model_path):
        ...
        trained_model = LitVGG19.load_from_checkpoint(checkpoint_callback.best_model_path)
    else:
    ...
    hparams_to_log = {
        ...
        "epochs": args.epochs,
        "model_type": "VGG19", # Updated model type
        "gpus": args.gpus,
        "nodes": args.nodes,
