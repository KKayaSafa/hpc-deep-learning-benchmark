...
class LitResNet50(L.LightningModule):
    def __init__(self, learning_rate=1e-2, num_classes=10, optimizer_name="sgd",
                 ...
                 rmsprop_alpha=0.99, rmsprop_eps=1e-8, rmsprop_weight_decay=0, rmsprop_momentum=0): # ReduceLROnPlateau parametreleri eklendi
        super().__init__()
    ...
    def configure_optimizers(self):
        ...
        if self.hparams.lr_scheduler_name == "step":
            ...
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer,
                mode='min' if 'loss' in self.hparams.lr_monitor_metric else 'max',
                factor=self.hparams.lr_reduce_factor,
                patience=self.hparams.lr_reduce_patience,
                verbose=True
            )
            scheduler_config["monitor"] = self.hparams.lr_monitor_metric
        else:
...
def main():
    ...
    parser.add_argument("--optimizer", type=str, default="sgd", choices=["sgd", "adam", "adamw", "rmsprop"], help="Optimizer to use: sgd | adam | adamw | rmsprop")
    ...
    parser.add_argument('--rmsprop_alpha', type=float, default=0.99, help='Smoothing constant for RMSprop (default: 0.99)')
    parser.add_argument('--rmsprop_eps', type=float, default=1e-8, help='Term added to denominator for numerical stability in RMSprop (default: 1e-8)')
    parser.add_argument('--rmsprop_weight_decay', type=float, default=0, help='Weight decay for RMSprop (default: 0)')
    parser.add_argument('--rmsprop_momentum', type=float, default=0, help='Momentum factor for RMSprop (default: 0)')
    ...
    logger = TensorBoardLogger(save_dir="./gpu_logs/", name="experiments_resnet50", default_hp_metric=False)
