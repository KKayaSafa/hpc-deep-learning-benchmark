...
from torchvision.models import efficientnet_b2 # Changed to EfficientNetB2
...
class LitEfficientNetB2(L.LightningModule): # Renamed class for EfficientNetB2
    def __init__(self, learning_rate=1e-2, num_classes=10, optimizer_name="sgd",
                 ...
                 lr_reduce_factor=0.1, lr_reduce_patience=10, lr_monitor_metric="val_loss",
                 # RMSprop specific parameters
                 rmsprop_alpha=0.99, rmsprop_eps=1e-8, rmsprop_weight_decay=0, rmsprop_momentum=0):
        super().__init__()
        self.save_hyperparameters()

        # Load EfficientNetB2
        weights_path = './model-weights/efficientnetb2_weights.pth' # Updated weights path
        model = efficientnet_b2(weights=None) # Load base EfficientNetB2 model
        
        if os.path.exists(weights_path):
            try:
                pretrained_state = torch.load(weights_path)
                model.load_state_dict(pretrained_state, strict=False)  # Load pretrained weights
                print(f"Loaded EfficientNetB2 weights from {weights_path}")
            except Exception as e:
                print(f"Warning: Could not load weights from {weights_path}: {e}")
                print("Using randomly initialized model.")
        else:
            print(f"Warning: EfficientNetB2 weights not found at {weights_path}. Using randomly initialized model.")
        
        # Modify the classifier for CIFAR10 (10 classes)
        # EfficientNet has a classifier layer that we need to modify
        num_features = model.classifier[1].in_features  # EfficientNet classifier structure: [Dropout, Linear]
    ...
    def configure_optimizers(self):
        ...
        if opt_name == "sgd":
            ...
            optimizer = torch.optim.RMSprop(
                ...
                weight_decay=self.hparams.rmsprop_weight_decay,
...
def main():
    ...
    parser.add_argument('--rmsprop_eps', type=float, default=1e-8, 
                        help='Term added to denominator for numerical stability in RMSprop (default: 1e-8)')
    parser.add_argument('--rmsprop_weight_decay', type=float, default=0, 
                        help='Weight decay for RMSprop (default: 0)')
    parser.add_argument('--rmsprop_momentum', type=float, default=0, 
                        help='Momentum factor for RMSprop (default: 0)')
    ...
    transform = transforms.Compose([
        transforms.Resize(288),  # EfficientNetB2 expects 288x288 images (slightly larger than B0's 224x224)
        transforms.ToTensor(),
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=16, pin_memory=True, shuffle=True)
    ...
    model = LitEfficientNetB2(learning_rate=args.learning_rate,
    ...
    logger = TensorBoardLogger(save_dir="./lightning_logs/", name="experiments_efficientnetb2", default_hp_metric=False)
    device_stats = DeviceStatsMonitor()
    gpu_logger = MultiGPUUtilizationLogger(log_frequency=100)

    checkpoint_dir = os.path.join("./out", f"checkpoints_efficientnetb2_{args.strategy}_{args.optimizer}_bs{args.batch_size}_lr{args.learning_rate}_sch{args.lr_scheduler_name}")
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="efficientnetb2-{epoch:02d}-{val_acc:.2f}"
    )
    ...
    trainer = L.Trainer(
        ...
        logger=logger,
        callbacks=[device_stats, early_stop_callback, checkpoint_callback, gpu_logger],
    ...
    if checkpoint_callback.best_model_path and os.path.exists(checkpoint_callback.best_model_path):
        ...
        trained_model = LitEfficientNetB2.load_from_checkpoint(checkpoint_callback.best_model_path)
    else:
    ...
    hparams_to_log = {
        ...
        "epochs": args.epochs,
        "model_type": "EfficientNetB2", # Updated model type
        "gpus": args.gpus,
        "nodes": args.nodes,
    ...
    # RMSprop parametrelerini loglama
    if args.optimizer == "rmsprop":
        hparams_to_log.update({
            "rmsprop_alpha": args.rmsprop_alpha,
            "rmsprop_eps": args.rmsprop_eps,
            "rmsprop_weight_decay": args.rmsprop_weight_decay,
            "rmsprop_momentum": args.rmsprop_momentum
